...
5. **Local IPC by Default**: Components 2 (Collector) and 3 (UI) frequently live on the same physical operator workstation, ~~talking via high-throughput localhost IPC (Unix Domain Socket or Windows Named Pipe)~~. Talking via a local TCP connection (gRPC/SSE/etc.) This way it is easy to just one day move the UI or collector to another machine (just change the IP address UI connects to and its done).
6. **Collecting each metric is opt-in**: For example, for one device it may not be important to log exactly which IPs it connects with, since the size of that kind of data can quickly get overwhelmingly big (or sometimes it may just not be relevant, like monitoring battery on a server), collecting each data source should be configurable.
---
...
flowchart TD
    subgraph Collectors["Collectors Subsystem"]
        ...
        NET["Network Collector<br/>• Per-NIC rx/tx bytes & packets<br/>• Drops, errors, collisions<br/>• Link speed, MTU, carrier, incoming/outgoing source/destination port/IPs"]
    end
...
* **Optional zstd compression**: Optionally compress data inside the batch assembler which helps both transport and file size. (Can also define a min. batch size) 
* **Strict Bounded Quota**: Configurable hard limit (default `1 GB`). When the volume is exhausted while offline, the oldest segment is purged (FIFO) to protect host storage.
* **Drain Protocol**: When a connection to the collector succeeds, the daemon initiates a draining stream from the oldest unacknowledged segment offset, in segments, removing each segment only when collector ack arrives. 
...
**When UI graph historical scopes are changed, live updates should also honor the LOD**: for example, if the scope is 5 mins, and the width of the graph allows 60 samples, then the average result of every 5 samples @1s is calculated and appended at the end once the correct number of samples arrive. In other words, graphs always stay live as long as the selected scope's END date is "NOW" (for custom mode). 
Pausing the graph movements when zoomed-in should be an optional feature in settings. (when paused, data is appended to the end but graph does not automatically pan itself, and a small magnifying glass icon is shown to indicate being zoomed in. clicking the icon resets zoom.)
...
flowchart TD
    subgraph Standard_Workstation["Scenario A: Operator Workstation (Co-located 2 & 3)"]
        UI_A["MADTOM Console / UI"] <-->|"Local TCP (gRPC)<br/>(Seamless IP migration)"| COL_A["madtom-collector"]
    end
...
3. **Step 3: Information Collector (`collector/`)**:
   ...
   * **Note that each collector can have multiple Nodes connected to it**.

4. **Step 4: C# Telemetry Client Integration (`src/Plugins/Telemetry/`)**:
   * `CollectorIpcDataProvider`: Connects to TCP endpoint (localhost or remote IP).
   ...
   * **Multi-Collector Client Support**: Each client supports adding multiple collectors. And each collector can have multiple nodes. Replace the color-code indicator on node cards with the **Collector Name**.
   * **Selective Subscription & Polling**:
     * Client chooses which node to listen to / fetch.
     * Client chooses what metrics are actively collected from each node (disabling an opt-in metric stops the daemon from querying it to save resources).
     * Client chooses what metrics are **streamed live** vs. **queried on-demand** for historical graphs.
