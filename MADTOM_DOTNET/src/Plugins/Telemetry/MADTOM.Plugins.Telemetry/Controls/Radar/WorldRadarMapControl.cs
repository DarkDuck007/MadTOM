...
public sealed class WorldRadarMapControl : Control
{
    ...
    private void LoadMapGeometries()
    {
        try
        {
            Uri uri = new("avares://MADTOM.Plugins.Telemetry/Assets/Maps/world-countries.json");
            if (!AssetLoader.Exists(uri))
            {
                uri = new Uri("avares://MadTOM/Assets/Maps/world-countries.json");
            }
            if (!AssetLoader.Exists(uri))
            {
                uri = new Uri("avares://MADTOM.Plugins.Telemetry/Assets/Maps/world-regions.json");
            }
            if (!AssetLoader.Exists(uri))
            {
