...
public sealed class LexiconService : ILexiconService
{
    ...
    public void LoadLexicon(string packName)
    {
        ...
        try
        {
            var uri = new Uri($"avares://MADTOM.Plugins.Telemetry/Assets/Lexicons/{packName.ToLowerInvariant()}.json");
            if (!AssetLoader.Exists(uri))
            {
                uri = new Uri($"avares://MadTOM/Assets/Lexicons/{packName.ToLowerInvariant()}.json");
            }
            if (AssetLoader.Exists(uri))
            {
